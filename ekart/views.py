...
def checkout(request):
    ...
    if request.method=="POST":
        ...
        # Format the ordered items for the email
        formatted_items = ""
        for key, value in dataitems.items():
            quantity, product_name, price = value
            formatted_items += f"- {product_name}: {quantity} x ₹{price} = ₹{int(quantity) * int(price)}\n"

        # Create the email message
            email_subject = "Order Placed Sucessfully"
            message = f"""Hello {name},

            Your order has been placed with Order ID {oid}.
            ...
            We will soon deliver your order to the below address:
